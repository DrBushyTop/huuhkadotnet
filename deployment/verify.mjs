...
for (const path of ['index.html', ...pages]) {
  ...
  if (path === 'index.html') {
    const blocked = readFileSync(`${site}/robots.txt`, 'utf8').includes('Disallow: /');
    assert.equal(!!document.querySelector('meta[name="robots"]')?.content.includes('noindex'), blocked, 'robots meta');
    assert.equal(!!response.headers.get('x-robots-tag')?.includes('noindex'), blocked, 'robots header');
  }
...
for (const rule of config.routes.filter(rule => rule.redirect).flatMap(rule => [rule, {...rule, route: rule.route.replace('/index.html', '/')}, {...rule, route: rule.route.replace('/index.html', '')}])) {
  const response = await request(new URL(rule.route, origin), {redirect: 'manual'});
